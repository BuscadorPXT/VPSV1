...
export function ExcelFilterDropdown({
  ...
  const sortedOptions = useMemo(() => {
    ...
    return emptyOption ? [emptyOption, ...nonEmptyOptions] : nonEmptyOptions;
  }, [options]);
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className={`
            justify-between h-7 sm:h-8 px-2 sm:px-3 text-xs font-normal w-full
            ${hasActiveFilters 

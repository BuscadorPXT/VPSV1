...
Cole o conteúdo do arquivo google-apps-script-otimizado.js
Configure a variável WEBHOOK_URL com sua URL da aplicação
Salve e teste editando uma célula
...
Verifique logs por polling residual
Aumente o TTL do cache para 30 minutos se aceitável
Considere rate limiting no webhook
Monitore quantos usuários estão conectados simultaneamente
...
Verifique logs em /tmp/logs/Start_application_*.log
Teste o webhook manualmente via curl
Verifique métricas em /api/cost-metrics/detailed
Revise o Google Apps Script no console do Google Sheets
Última Atualização: 26 de Outubro de 2025 Status: ✅ Implementado e Testado Economia Esperada: 70-85% (~$628/mês